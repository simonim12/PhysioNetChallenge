...
an aggregate dataset (beats_dataset.npz) and beats_manifest.csv.

Usage (WFDB header available next to .dat):
  python bulk_one_beat.py --root C:\path\to\root --outdir C:\path\to\out

Raw fallback (no .hea): you MUST provide fs/nch/etc; set --raw-if-missing:
  python bulk_one_beat.py --root C:\path\to\root --outdir C:\out --raw-if-missing \
    --fs 360 --nch 1 --lead 0 --dtype int16 --endian little

Common flags:
  --pre 0.25 --post 0.45 --target-len 300 --lead 0 --lead-name II --which-r first|middle
"""
...
def bandpass_qrs(x, fs, lo=5.0, hi=35.0):
    ...
    # very simple fallback: detrend by moving average + short smoothing
...
# ---------- core per-file ----------
def process_one_file(dat_path, out_root, root, pre_sec, post_sec, target_len,
                     which_r, prefer_lead, prefer_lead_name, raw_if_missing,
                     raw_fs, raw_nch, raw_dtype, raw_endian):
    """
    Returns dict with success flag and paths.
    """
    ...
    try:
        ...
        return {"ok": True, "beat": beat, "meta": meta, "outdir": outdir}
    except Exception as e:
        return {"ok": False, "error": str(e), "path": dat_path}


# ---------- CLI ----------
def main():
    ap = argparse.ArgumentParser(description="Recursively create ONE beat per .dat file.")
    ...
    for i, dp in enumerate(sorted(dat_files)):
        ...
        if res.get("ok"):
            beats.append(res["beat"])
            m = res["meta"]
            rows.append({
                "rel_path": m["rel_path"],
                "fs": m["fs"],
                "lead_index": m["lead_index"],
                "lead_name": m["lead_name"],
                "r_time_s": m["r_time_s"],
                "target_len": m["target_len"],
                "outdir": res["outdir"].replace("\\","/"),
                "mode": m["mode"],
            })
            print(f"[{i+1}/{len(dat_files)}] OK: {rel}")
        else:
            failed.append((rel, res.get("error")))
            print(f"[{i+1}/{len(dat_files)}] FAIL: {rel} -> {res.get('error')}")

    # Save aggregate dataset
    if beats:
        all_beats = np.vstack(beats)
        np.savez_compressed(os.path.join(out_root, "beats_dataset.npz"), beats=all_beats)
        import pandas as pd
        pd.DataFrame(rows).to_csv(os.path.join(out_root, "beats_manifest.csv"), index=False)
        print(f"\nSaved aggregate dataset: beats_dataset.npz (shape {all_beats.shape})")
        print(f"Saved manifest CSV:      beats_manifest.csv  (rows {len(rows)})")

    if failed:
        # Write failures for debugging
        with open(os.path.join(out_root, "failed.json"), "w", encoding="utf-8") as f:
            json.dump([{"file": f, "error": e} for f, e in failed], f, indent=2)
        print(f"\nSome files failed: see failed.json (count={len(failed)})")
